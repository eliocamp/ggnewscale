bump_aes_layer <- function(layer, original_aes, new_aes) {
  ...
  old_aes <- names(new_layer$mapping)[
    names(new_layer$mapping) %in% original_aes
  ]

  # If not explicit, get the default
  if (length(old_aes) == 0) {
    old_aes <- names(new_layer$stat$default_aes)[
      names(new_layer$stat$default_aes) %in% original_aes
    ]
    if (length(old_aes) == 0) {
      old_aes <- names(new_layer$geom$default_aes)[
        names(new_layer$geom$default_aes) %in% original_aes
      ]
    }
  }
  ...
  new_setup_data <- function(self, data, scales, ...) {
    ...
    colnames(data)[colnames(data) %in% new_aes] <- original_aes
    data <- ggplot2::ggproto_parent(self$super(), self)$setup_data(
      data,
      scales,
      ...
    )
  ...
  attributes_replace <- attributes_old[setdiff(
    names(attributes_old),
    names(attributes_new)
  )]
